...
/* حالةُ العدّاد بين النداءات — لازمةٌ للرتابة داخل المللي الواحدة */
let lastMs = -1;
let seq = 0;
 ...
 *
 * الزمنُ وحدَه لا يكفي: عشراتُ المعرّفات تُولَّد في المللي الواحدة فتتساوى
 * طوابعُها ويصير ترتيبُها عشوائيّاً. لذلك تحمل بتّاتُ rand_a الاثنتا عشرة
 * **عدّاداً** يتزايد داخل المللي ويُبذَر عشوائيّاً متى تقدّم الزمن — وهي
 * الطريقةُ الأولى في RFC 9562 §6.2. والبذرةُ في النصف الأدنى ليبقى للعدّاد
 * متّسعٌ ٢٠٤٨ خطوةً قبل أن يلتفّ.
 */
export function uuidV7(ms: number): string {
  const b = crypto.getRandomValues(new Uint8Array(16));
  if (ms === lastMs) seq = (seq + 1) & 0x0fff;
  else { lastMs = ms; seq = ((b[6] << 8) | b[7]) & 0x07ff; }
  const t = BigInt(ms);
